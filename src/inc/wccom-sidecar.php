...
add_action( 'sift_for_woo_sift_action_trust_list_payment_abuse', __NAMESPACE__ . '\unblock_user_from_purchases', 10, 1 );
add_action( 'sift_for_woo_sift_action_looks_good_payment_abuse', __NAMESPACE__ . '\unblock_user_from_purchases', 10, 1 );
add_action( 'sift_for_woo_sift_action_not_likely_fraud_payment_abuse', __NAMESPACE__ . '\unblock_user_from_purchases', 10, 1 );

add_action(
	'sift_for_woo_sift_action_likely_fraud_refundno_renew_payment_abuse',
	function ( $user_id ) {
...
add_action(
	'sift_for_woo_sift_action_likely_fraud_keep_purchases_payment_abuse',
	function ( $user_id ) {
		update_user_meta( $user_id, 'is_blocked_from_purchases', true );
		display_sgdc_error( 'You are blocked from making purchases due to a recent fraud review. SGDC Error OYBPXRQ' );
	},
...
add_action(
	'sift_for_woo_sift_action_fraud_payment_abuse',
	function ( $user_id ) {
		update_user_meta( $user_id, 'is_blocked_from_purchases', true );
		void_and_refund_user_orders( $user_id );
		cancel_and_remove_user_subscriptions( $user_id );
...
add_action(
	'sift_for_woo_sift_action_block_wo_review_payment_abuse',
	function ( $user_id ) {
		update_user_meta( $user_id, 'is_blocked_from_purchases', true );
	},
	10,
