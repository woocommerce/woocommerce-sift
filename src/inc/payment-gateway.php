...
class Payment_Gateway extends Sift_Property {
	...
	public function __construct( string $woo_gateway_id, \WC_Order $order ) {
		if ( 'transact' === strtolower( $woo_gateway_id ) ) {
	...
	public static function normalize_payment_gateway_string( string $gateway_id, \WC_Order $order ): ?string {
		$payment_gateway = apply_filters( sprintf( 'sift_for_woocommerce_%s_payment_gateway_string', $gateway_id ), $gateway_id, $order ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.DynamicHooknameFound
